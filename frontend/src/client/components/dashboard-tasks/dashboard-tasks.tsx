import { Calendar, MoreVertical, PenLine, Trash2 } from "lucide-react";
import React, { useEffect, useRef } from "react";
import { TaskInterface } from "./interface";
import { API_BASE_URL, ENDPOINTS } from "@/URI";

export const TaskBanner = ({ id, title, tags, dueDate, priority, assignee, description, draggedRef, columnId, openUpdateModal, status }: any) => {
  const taskRef = useRef<HTMLDivElement>(null);
  useEffect(() => {
    if (taskRef.current) {
      taskRef.current.addEventListener('dragstart', (e) => {
        const taskData = JSON.stringify({ taskDetails: { id, title, tags, dueDate, priority, assignee, description, status }, colOld: columnId })
  ...
  const handleDeleteTask = async (e: React.MouseEvent<HTMLButtonElement>) => {
    ...
    e.stopPropagation();
    try {
      let response = await fetch(`${API_BASE_URL}${ENDPOINTS.DELETE.deleteTask(id.toString())}`, {
        method: 'DELETE',
        headers: {
  ...
  return (
    ...
    >
      <div className="flex items-start justify-between mb-3">
        ...
        <button className="text-gray-600 hover:text-gray-400 transition-colors opacity-0 group-hover:opacity-100 px-2"
          onClick={(e) => openUpdateModal(e, { id, title, tags, dueDate, priority, assignee, description, status })}>
          <PenLine className="w-4 h-4" />
        </button>
        <button className="text-gray-600 hover:text-gray-400 transition-colors opacity-0 group-hover:opacity-100 ml-2"
      ...
      <div className="flex items-center justify-between pt-3 border-t border-gray-800">
        <div className="flex items-center gap-3">
          ...
          <span className={`px-2 py-0.5 rounded text-xs border ${getPriorityColor(priority.toLowerCase())}`}>
            {priority}
          </span>
        </div>
