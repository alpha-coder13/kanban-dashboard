...
import { TaskUpdate } from "./components/dashboard-updateTask/dashboard-updateTask";
...
export default function Homepage() {
  ...
  const dragDropTasks = async (taskData: TaskInterface, columnIdNew: number, columnIdOld: number) => {
    ...
    taskData.status = STATUS_CODE_TO_STATUS_TEXT[columnIdNew];
    let updateTaskResponse = await updateTask({ ...taskData });
    if (updateTaskResponse.data) {
      const updateTasksStates = (prevState: any) => {
        ...
        if (newState[columnIdNew].indexOf((val: any) => { console.log(val); return parseInt(val.id) === parseInt(`${taskData.id}`) }) == -1) {
  ...
  const STATUS_CODE_TO_STATUS_TEXT: Record<number, string> = {
    1: 'To Do'.toUpperCase(),
  ...
  const getTasks = async () => {
    fetch(API_BASE_URL + ENDPOINTS.GET.getTasks(undefined), {
      method: 'GET',
      headers: {
      ...
      .then(data => {
        ...
        data.forEach((val) => {
          ...
          val.dueDate = new Date(val.dueDate).toISOString().split('T')[0];
  ...
  const GetModalChildTSX = modalChild === 'newTask' ? AddTask : modalChild === 'filterTask' ? TaskFilter : modalChild === 'updateTask' ? TaskUpdate : () => (<></>);
  useEffect(() => {
    getTasks();
  }, []);
  const [updateTaskData, setUpdateTaskData] = useState<any>(null);
  const openUpdateModal = (e: React.MouseEvent<HTMLButtonElement>, task: any) => {
    e.preventDefault();
    e.stopPropagation();
    setUpdateTaskData(task);
    setModalChild('updateTask');
    onClose(undefined);
  }
  return (
    <>
      {modal.state.isOpen && (<div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
        <GetModalChildTSX onClose={(e: any) => onClose(e)} getTasks={getTasks} task={updateTaskData} />
      </div>)
      }
            ...
            {columns.map((column: any) => {
              return (
                <Columns id={parseInt(column.id)} key={column.id} title={column.title} color={column.color} count={(column.count).toString()} draggedRef={draggedRef} dragDropTasks={dragDropTasks}>
                  {tasks[parseInt(column.id)]?.map(({ id, tags, description, dueDate, priority, assignee, title, status }: TaskInterface) => (
                    <TaskBanner columnId={column.id} id={id} key={(id).toString()} tags={tags} description={description} dueDate={dueDate} priority={priority} assignee={assignee} title={title} draggedRef={draggedRef} openUpdateModal={openUpdateModal} status={status} />
                  ))}
                </Columns>
              )
            }
